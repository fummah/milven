...
- VOLUME / MODULE CONTEXT: The vignette MUST be explicitly grounded in the volume "${volumeName}". The opening line MUST be "${volumeName}". The case study content, protagonist's role, and business scenario must directly reflect the volume topic area.
- SUB-QUESTION GROUNDING RULE: Every sub-question MUST directly reference specific information, data, or exhibits from the vignette. No sub-question can ask about something not present in the vignette. For each sub-question, verify: "Can a candidate derive the answer using ONLY information provided in the vignette above?" If No, rewrite the sub-question.
- CALCULATION INPUT VERIFICATION: Every number used in workedSolution must appear EXPLICITLY in the vignetteText or an exhibit. After writing all 4 sub-questions, trace each number in each workedSolution back to its source in the vignette. If any input value is NOT explicitly stated in the vignette, add it to the vignette. Never assume the candidate knows standard values (e.g. risk-free rate, tax rate, market premium) — state them in the vignette.
...
- TWO TABLE TYPES — choose the right style based on content:
  ...
  CRITICAL TABLE STRUCTURE RULE — MULTIPLE COLUMNS: Data tables MUST use SEPARATE COLUMNS for each data attribute. NEVER cram all information into a single column. For example, a fund performance table MUST have columns like: Fund Type | Current Value | Commitment — NOT one column with "Venture Capital: Current Value: $120 million, Commitment: $80 million". Each distinct piece of data (name, value, ratio, date, amount, return, weight) gets its OWN column with a proper <th> header. Minimum 2 columns for any data table, typically 3-5 columns.
...
1. FIRST write the complete workedSolution with every step shown, using ONLY numbers explicitly stated in the vignette.
...
11. FINAL ACCURACY VERIFICATION: Read each sub-question's workedSolution from start to finish. Check that every input number exists in the vignette. Recompute the final answer. Confirm isCorrect:true matches the recomputed answer. If ANY mismatch is found, fix the workedSolution or correct option BEFORE returning JSON.
...
Each object in "items" MUST follow this structure:
{
  "vignetteText": string — A LONG, RICH CFA-EXAM-STYLE CASE STUDY PASSAGE (MINIMUM 250 prose words, target 250-650). IMPORTANT: word count means PROSE words only — HTML tags, table markup, style attributes, and exhibit labels do NOT count. When tables are included, write MORE narrative prose around them to compensate. Requirements:
    ...
    • VOLUME / MODULE GROUNDING: The case study MUST be explicitly tied to "${volumeName}". The business scenario, protagonist's role, and all content must directly relate to this volume's topic area.
    ...
    • SUB-QUESTION GROUNDING RULE: Every sub-question MUST directly reference specific data or exhibits from this vignette. No sub-question can ask about something not present. For each: "Can the candidate answer using ONLY information above?"
    • CALCULATION INPUT RULE: Every number used in any sub-question's workedSolution MUST appear EXPLICITLY in the vignetteText or an exhibit. After writing all 4 sub-questions, trace every input value back to its source in the vignette. Never assume standard values — state them.
...
`}
    ...
    • CRITICAL — ANSWER CONSISTENCY RULE (MUST FOLLOW):
      ...
      7. FINAL ACCURACY VERIFICATION: Read each sub-question's workedSolution. Trace every input number back to the vignette. Recompute the final answer. Verify isCorrect matches the recomputed answer. Fix any mismatches.
    ...
    • LOS MATCHING RULE: Each sub-question's "los" MUST match the LEARNING OUTCOME STATEMENT for the specific topic that sub-question tests. Do NOT reuse the same LOS across sub-questions — each sub-question must have its own LOS matching its specific concept/topic.
